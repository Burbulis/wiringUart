...
//		_LOG::out_s << "SIGINT deteced try to stop execution." 	<< logPath << std::endl;
...
size_t getUartDataLoop(const config& conf)
{
	...
	if (serial < 0) 
	{
		_LOG::out_s << "can not opened serial ret value =" << serial << std::endl;
		throw (std::runtime_error(_LOG::out_s.str()));
	}

    FILE *fstream = fopen(conf.outFileName().c_str(),"ab+");
	if (!fstream)
	{
	...
	while(running)
	{
	    szData = serialDataAvail(serial);
	    if (szData == 0)
			continue;
	    if (buffer.size()< szData)
			buffer.resize(szData * 2);
	    size_t readed = read(serial,&buffer[0],szData);
	...
	fclose(fstream);
    LOGTOFILE(LOGHTML::messageType::STRONG_WARNING,"FINAL READING LOOOP");
	return (total);
}
void reconfig(config *conf)
{
	...
	std::filesystem::path _logPath = std::filesystem::path(logPath);
...
int main(void)
{
	...
	if (wiringPiSetup() < 0) 
	{
		LOGTOFILE(LOGHTML::messageType::STRONG_WARNING,std::string("wrirng setup error!"));
		return  (EXIT_FAILURE);
	}
